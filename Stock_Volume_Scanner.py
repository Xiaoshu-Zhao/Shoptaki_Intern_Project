...
if __name__ == '__main__':
    result = mainObj().main_func()


def barGraph(symbol,startdate):
    import yfinance as yf
    data = yf.download(symbol, start=startdate,end=date.today(), progress=False)
    data.reset_index(level=0, inplace=True)
    tempList = []
    for x in data['Date']:
    ...
    data['Volume'].plot(kind='bar', ax=ax)
    plt.show()

data = barGraph("CHNGU",'2020-10-01')
